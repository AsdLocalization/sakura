...
zh_hant.MultiUser=互換モードでインストールします．システムファイルおよびプログラムをユーザ権限で編集すると編集結果が他のアプリケーションから参照できないことがあります．(VirtualStore機能)

en.InitWiz_Title=Selection of Preference preservation method
ja.InitWiz_Title=設定保存方法の選択
zh_hans.InitWiz_Title=配置文件保存位置
zh_hant.InitWiz_Title=設定保存方法の選択
...
zh_hant.InitWiz_SubTitle=設定をユーザ毎に保存するか実行ファイルの場所へ保存するかを選択してください
...
zh_hant.InitWiz_Comment=よくわからない場合は変更しないでください
...
zh_hant.InitWiz_Check=設定をユーザ毎に個別に管理する
...
zh_hant.ReadyMemo_SaveLocation=設定ファイル等格納場所
...
zh_hant.ReadyMemo_VirtualStoreEnable=有効
